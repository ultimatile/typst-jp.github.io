...
レイアウトのガイドラインは学会ウェブサイトに掲載されています。
ここでは以下の条件である場合を考えましょう。
...
ここでは、ページ番号の付け方を定義する[番号付けパターン]($numbering)を指定できます。
...
タイトルがすでに太字になっており、周囲に空白があることがわかります。
...
思い出してみると、タイトルを中央揃えにし、17ptの大きさにしたいのでした。
...
行は自動的に追加されますが、`rows`引数を用いて手動でサイズを指定できます。
...
学会は、アブストラクトを両端揃えなしで中央揃えに設定することを求めていることを思い出してください。
