...
この例は前の例と同じ結果になりますが、まずCSVを読み込んでから、`map`で各行を変換します。`map`に渡した関数はデータの各行に適用され、元の行を置き換える新しい配列を返します。ここでは残したい列が3列目と4列目なので、`{.slice(2, 4)}`でその2列だけを取り出しています。`moore`は2次元の配列（各行自体が配列）であるため、`map`の結果も入れ子になった配列のままです。`flatten`関数はこの入れ子構造を1次元の配列に変換します。これは、データを`table`関数へ展開する際に必要です。最後に、各行からちょうど2列を残すため、`{columns: 2}`を明示的に指定しています。
