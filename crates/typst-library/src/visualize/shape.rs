...
pub struct CircleElem {
    /// 円の半径。`width`および`height`とは排他です。
    #[external]
    pub radius: Length,

    /// 円の幅。`radius`および`height`とは排他です。
    ///
    /// `radius`とは異なり、こちらは親コンテナーの幅に対する相対指定が可能です。
    #[parse(
        let size = args
    ...
    /// 円の高さ。`radius`および`width`とは排他です。
    ///
    /// `radius`とは異なり、こちらは親コンテナーの高さに対する相対指定が可能です。
    #[parse(match size {
        None => args.named("height")?,
        size => size.map(Into::into),
    })]
    ...
    /// レイアウトに影響を与えずに円の大きさを拡大する量。
    /// 詳細は[boxのドキュメント]($box.outset)を参照してください。
    #[fold]
    pub outset: Sides<Option<Rel<Length>>>,

    /// 円に配置するコンテンツ。
    /// 円はこのコンテンツにあわせて、1:1のアスペクト比を保ったまま拡大します。
    #[positional]
    pub body: Option<Content>,
}
