...
impl Counter {
    ...
    /// 番号付けされたカウンターの現在の値を表示および戻り値としての整形された出力。
    #[func(contextual)]
    pub fn display(
        self,
        engine: &mut Engine,
    ...
    /// 文書中に出力がなければ何も起こりません！
    /// これは例えば`{let _ = counter(page).step()}`と書いた場合が何も起きないときに該当します。
    ...
    /// カウンターの値の更新。
    ///
    /// `step`と同様に生成されたコンテンツが文書中に配置されたときに限り更新が行われます。
    #[func]
    pub fn update(
        self,
        span: Span,
        /// 整数または整数の配列が与えられた場合、カウンターをその値に設定します。
        /// 関数を与える場合、その関数は（各数値を別々の引数として）前のカウンターの値を受け取り、新しい値を（整数または配列で）返さなければなりません。
