...
このプロジェクトは、[Typst GmbH](https://typst.app/legal/)の許諾を得て、最新の[公式のドキュメント](https://typst.app/docs/)より翻訳することで、非公式な日本語ドキュメントを提供することを目的としています。まさに、あなたのようなボランティアの皆様のご協力の元、成り立っています。当ガイドラインをご一読の上、翻訳・校正・提案およびその他の作業にご参加いただければ幸いです。
...
4. 翻訳作業の途中でも、Draft Pull Requestを作成して、翻訳の進捗状況を共有できます。
...
[翻訳ガイドライン](https://github.com/typst-jp/docs/blob/main/TRANSLATING_GUIDELINES.md)に従った体裁かどうかを確認する場合は、以下のコマンドを実行します。
...
現在、Rustのソースコードを直接textlintで解析できません。これは、Rustのrustdocに対応したtextlintプラグインがまだ存在しないためです。そのため、当面の対応として、出力されたHTMLファイルをtextlintで解析しています。
...
Dockerコンテナー上に上記と同一の環境を構築して作業できます。
